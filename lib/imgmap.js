...
function imgmap(config) {
    ...
    this.isMSIE    = (navigator.appName == "Microsoft Internet Explorer"); // navigator.appName == "Microsoft Internet Explorer" || navigator.appName === 'Netscape' || ua.indexOf('Windows NT') !== -1;
...
imgmap.prototype.loadImage = function (img, imgw, imgh) {
    ...
    if (typeof img == 'string') {
        ...
        if (typeof this.pic == 'undefined') {
            ...
            //black magic to send "this" context to img_xxxx event handlers
            $(this.pic)
                .on('mousedown', $.proxy(this.img_mousedown, this))
                .on('mouseup', $.proxy(this.img_mouseup, this))
                .on('mousemove', $.proxy(this.img_mousemove, this));
		...
		if (this.areas[i]) {
			if (this.areas[i].shape && this.areas[i].shape != 'undefined') {
				...
				html+= '<area shape="' + this.areas[i].shape + '"' +
				    ...
				    ' data-editor-url="' + this.areas[i].ahref + '" />';
			}
		}
	}
	//alert(html);
...
imgmap.prototype.img_mousedown = function (e) {
    ...
    // GASTON MORENO 08/07/2014
    // prevents reference to an existent area when we try to draw a new one
    if (this.currentid < this._getLastArea().aid) {
        this.currentid = this._getLastArea().aid;
    }
    ...
    if (this.is_drawing == this.DM_POLYGON_DRAW || this.is_drawing == this.DM_BEZIER_DRAW) {
        ...
        // GASTON MORENO 08/08/2014: Added viewmode = 1 when user ends the draw of a shape
        this.viewmode = 1;
        this.is_drawing = 0;
        this.statusMessage(this.strings.READY);
        this.relaxArea(this.currentid);
...
imgmap.prototype.area_mousemove = function(e) {
    ...
    if (!this.is_drawing) {
        ...
        // GASTON MORENO 07/29/2014: DEACTIVATE PLUGIN AREA MOVE/RESIZE
...
/*		var xdiff = (this.isMSIE) ? (window.event.offsetX) : (e.layerX);
        ...
        // GASTON MORENO 07/29/2014: TO PREVENT LOOSEN ENDS EXCEPTIONS
        if (!this.memory[this.currentid]) {
            return;
        }
 ...
 *	@param	e	The event object 
 */
imgmap.prototype.area_mousedown = function (e, area) {
	...
	if (!this.is_drawing) {
        ...
        if (area) {
            ...
            obj = (this.isMSIE) ? window.event.srcElement : e.currentTarget;
        }
	    ...
	    // GASTON MORENO 08/04/2014 Cancel Stop Propagation: Now we need the event to bubble so we can use draggable
	    /*
		if (this.isMSIE) {
			window.event.cancelBubble = true;
		}
